...
export const COLUMN_SPACING = 130; // ค่าอ้างอิงเต็ม (ไม่ได้ใช้วาดเส้นไหนตรง ๆ แล้ว — ทุกช่วงคอลัมน์ใช้ครึ่งนี้หมด ดู HALF_COLUMN_SPACING)
const HALF_COLUMN_SPACING = COLUMN_SPACING / 2; // ระยะห่างแนวนอนของทุกช่วงคอลัมน์ (รอบ1เข้ารอบ2, รอบ2เข้ารอบ4, ฯลฯ) — รอบ 1 เองไม่มีเส้นเข้าอยู่แล้ว (เป็นรอบแรก) จึงไม่มีอะไรให้สั้นลง แต่ทุกช่วงถัดจากนั้นสั้นลงครึ่งเท่ากันหมด ไม่มีข้อยกเว้น
...
// คืนระยะสะสม (x แบบ relative เริ่มที่ 0) ของแต่ละคอลัมน์ในฝั่งเดียว — ทุกช่วงคอลัมน์ (รวมรอบ1เข้ารอบ2) สั้นลงครึ่งเท่ากันหมด
function cumulativeColumnOffsets(colCount) {
  const offsets = [0];
  for (let i = 1; i < colCount; i++) {
    offsets.push(offsets[i - 1] + HALF_COLUMN_SPACING);
  }
  return offsets;
}
